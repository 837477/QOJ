...
#문제 반환
def get_myproblem(db, user_id, p_id):
    result = QOJ__join_query(db).find__myproblem(user_id, p_id)
...
def delete_testdb(QOJ_db, testDB_db, mt_id):
    ...
    QOJ__testDB(testDB_db).execute_query(query)
...
def query_execute(QOJ_db, testDB_db, query, class_id):
    ...
    if table_name:
        query = query.replace(table_name['tableName'], ("QOJ$" + class_info['user_id'] + "$" + table_name['tableName']))

    #테스트 디비에 실행!
    try:
        result = QOJ__testDB(testDB_db).execute_query(query)
    except Exception as e:
        result = str(e)
        temp = result.split('$')
        temp2 = temp[0].split(' ')
        result = temp2[:-1] + ["'"] + temp[-1:]
        result = ' '.join(result)
...
def query_submit(QOJ_db, testDB_db, JWT, query, class_id, p_id):
    ...
    if table_name:
        user_query = user_query.replace(table_name['tableName'], ("QOJ$" + class_info['user_id'] + "$" + table_name['tableName']))
    ...
    #사용자 쿼리 테스트 디비에 실행!
    try:
        user_result = QOJ__testDB(testDB_db).execute_query(user_query)
    except Exception as e:
        user_result = str(e)
        temp = user_result.split('$')
        temp2 = temp[0].split(' ')
        user_result = temp2[:-1] + ["'"] + temp[-1:]
        user_result = ' '.join(user_result)
        return user_result
    ...
    #정답 쿼리 테스트 디비에 실행!
    admin_result = QOJ__testDB(testDB_db).execute_query(query_answer)
